...
Key pair authentication uses two keys: a private key (kept secret) and a public key (shared with Snowflake). You add the public key to your Snowflake user account, then use the private key to authenticate.

This is more secure than passwords because the private key never leaves your secure environment - no secrets are transmitted over the network. Instead, your private key signs a JSON Web Token (JWT) that proves your identity to Snowflake, which then verifies the signature using your public key.
...
For testing, you can generate a key pair locally using [OpenSSL](https://docs.snowflake.com/en/user-guide/key-pair-auth#generate-the-private-key) (on macOS/Linux) or OpenSSH (on all platforms, including Windows PowerShell).

To generate a private key using OpenSSH, run the following command in your terminal:
...
```bash
ssh-keygen -f rsa_key.p8 -e -m PKCS8 > rsa_key.pub
```
  </TabItem>
  <TabItem value="windows-powershell" label="Windows PowerShell">
```powershell
ssh-keygen -f rsa_key.p8 -e -m PKCS8 | Out-File -FilePath rsa_key.pub -Encoding ASCII
```
  </TabItem>
</Tabs>
