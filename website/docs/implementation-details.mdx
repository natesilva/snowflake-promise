...
To add Promise support to the Snowflake SDK `Connection`, we use a [JavaScript Proxy](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) to intercept and modify method calls.
...
Our wrapper detects whether a callback is provided:
- **With callback**: Runs the original SDK method unchanged, returns `void`.
...
Traditional Promise APIs return a single async value, making this pattern difficult to implement cleanly.

### The Solution Pattern

Our approach is to return an object containing both the immediate value and a Promise for the async result:
...
This maintains full callback API functionality while enabling modern async/await patterns.
