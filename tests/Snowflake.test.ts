...
import { ConnectionOptions } from "../src/types/ConnectionOptions";
import {
  mockConnection,
  mockSdk,
  resetMocks,
...
describe("Snowflake", () => {
  ...
  describe("constructor", () => {
    ...
    it("should configure SDK with logLevel if provided", () => {
      const snowflake = new Snowflake(connectionOptions, { logLevel: "debug" });
      expect(mockSdk.configure).toHaveBeenCalledWith({ logLevel: "debug" });
    });

    it("should handle configureOptions if provided as an object", () => {
      const configureOptions = { ocspFailOpen: true };
      const snowflake = new Snowflake(connectionOptions, {}, configureOptions);
    ...
    it("should warn but not fail if configureOptions is provided as a boolean", () => {
      ...
      const snowflake = new Snowflake(connectionOptions, {}, true);
  ...
  describe("connectAsync", () => {
    ...
    it("should reject if connection fails", async () => {
      ...
      const snowflake = new Snowflake(connectionOptions);
      await expect(snowflake.connectAsync()).rejects.toThrow(
        "Connection failed"
      );
    });
  });
